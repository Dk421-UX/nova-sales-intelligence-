...
async function runAuthSuite() {
  ...
  // 1. Valid Admin Login (Default Demo Credentials)
  ...
  assert(res1.status === 200, 'Valid admin login returns 200 OK');
  ...
  // 2. Valid Admin Login with Email & Canonical Password
  const res2 = await fetch(`${baseUrl}/auth/login`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ username: 'admin@novalifespace.in', password: 'AdminPassword2026!' })
  });
  const body2 = await res2.json();

  assert(res2.status === 200, 'Admin login with email and canonical password returns 200 OK');
  ...
  assert(res3.status === 200, 'Valid staff login returns 200 OK');
